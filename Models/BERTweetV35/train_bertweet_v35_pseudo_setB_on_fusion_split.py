...
PSEUDO_SET_B_PATH = os.path.join(
    outputs_dir,
    "hybrid_v4",
    "supplement_audit",
...
bertweet_v35_output_dir = os.path.join(outputs_dir, "bertweet_v3_5_controlled_sampling")
bertweet_v35_checkpoints_dir = os.path.join(bertweet_v35_output_dir, "checkpoints")
bertweet_v35_logs_dir = os.path.join(bertweet_v35_output_dir, "logs")
bertweet_v35_metrics_dir = os.path.join(bertweet_v35_output_dir, "metrics")


def load_supplement_pseudo_rows(pseudo_labels: Dict[str, str]) -> List[dict]:
    pseudo_ids = set(pseudo_labels.keys())
    rows = []

    for row in iter_ndjson(bertweet_v35_supp_tokenized_path):
        cid = str(row["celebrity_id"])

        if cid not in pseudo_ids:
            continue

        row = dict(row)
        row["occupation"] = pseudo_labels[cid]
        row["pseudo_label"] = pseudo_labels[cid]
        row["is_pseudo"] = True
        row["pseudo_source"] = "supplement_setB_conf0p45_margin0p10_max250_no_politics"

        rows.append(row)

    print(f"[INFO] Loaded supplement pseudo tokenized rows: {len(rows)}")
    print(f"[INFO] Pseudo celebrities in loaded rows: {len(set(str(r['celebrity_id']) for r in rows))}")

    return rows

def load_pseudo_set_b_ids_and_labels():
    if not os.path.exists(PSEUDO_SET_B_PATH):
        raise FileNotFoundError(f"Missing pseudo Set B file: {PSEUDO_SET_B_PATH}")

    with open(PSEUDO_SET_B_PATH, "r", encoding="utf-8") as f:
        rows = json.load(f)

    pseudo_labels = {
        str(row["celebrity_id"]): str(row["pseudo_label"])
        for row in rows
    }

    print(f"[INFO] Loaded pseudo Set B examples: {len(pseudo_labels)}")
    print(f"[INFO] Pseudo distribution: {Counter(pseudo_labels.values())}")

    return pseudo_labels

def ensure_dirs():
    os.makedirs(hybrid_v4_bertweet_probs_dir, exist_ok=True)
    os.makedirs(bertweet_v35_output_dir, exist_ok=True)
    os.makedirs(bertweet_v35_checkpoints_dir, exist_ok=True)
    os.makedirs(bertweet_v35_logs_dir, exist_ok=True)
    os.makedirs(bertweet_v35_metrics_dir, exist_ok=True)
...
def load_train_rows_for_split(target: str, train_ids: set, val_ids: set) -> List[dict]:
    ...
    for row in iter_ndjson(bertweet_v35_train_tokenized_path):
        cid = str(row["celebrity_id"])
    ...
    print(f"[INFO] Loaded V3.5 train-tokenized rows for fusion split: {len(rows)}")
...
def load_test_rows(target: str) -> List[dict]:
    ...
    for row in iter_ndjson(bertweet_v35_test_tokenized_path):
        if target == "occupation_3class" and row["occupation"] == "creator":
            continue
        rows.append(row)

    print(f"[INFO] Loaded V3.5 official test rows: {len(rows)}")
...
def build_training_args(target: str):
    output_dir = os.path.join(
        bertweet_v35_checkpoints_dir,
        f"hybrid_v4_fusion_split_{target}",
    )
    logging_dir = os.path.join(
        bertweet_v35_logs_dir,
        f"hybrid_v4_fusion_split_{target}",
    )
...
def train_one_target(target: str):
    print(f"\n========== HybridV4 BERTweet V3.5 on fusion split: target={target} ==========")
    ...
    if target == "occupation":
        pseudo_labels = load_pseudo_set_b_ids_and_labels()
        pseudo_rows = load_supplement_pseudo_rows(pseudo_labels)
        train_tokenized_rows.extend(pseudo_rows)
    else:
        pseudo_rows = []
    grouped, _ = group_rows_by_celebrity(train_tokenized_rows, target)

    pseudo_ids_sorted = sorted(
        [cid for cid in pseudo_labels.keys() if cid in grouped],
        key=lambda x: int(x) if x.isdigit() else x,
    ) if target == "occupation" else []
    ...
    train_ids_sorted = train_ids_sorted + pseudo_ids_sorted
    ...
    val_rows_predict = prepare_prediction_rows(
        grouped=grouped,
        celebrity_ids=val_ids_sorted,
        max_chunks_per_celebrity=MAX_PREDICT_CHUNKS_PER_CELEB,
    )
    ...
    val_predictions, val_metrics = aggregate_celebrity_predictions(
        ...
        prob_key="bertweet_v35_probabilities",
        pred_key="bertweet_v35_pred_label",
        split="fusion_val",
    )
    ...
    val_pred_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_fusion_val_bertweet_v35_probs.json",
    )
    test_pred_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_test_bertweet_v35_probs.json",
    )
    val_metrics_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_fusion_val_bertweet_v35_metrics.json",
    )
    test_metrics_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_test_bertweet_v35_metrics.json",
    )
    ...
    save_json(
        ...
        os.path.join(bertweet_v35_metrics_dir, f"{target}_fusion_split_report.json"),
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train HybridV4 BERTweet V3.5 controlled-sampling model on fusion split"
    )
    parser.add_argument(
        "--target",
