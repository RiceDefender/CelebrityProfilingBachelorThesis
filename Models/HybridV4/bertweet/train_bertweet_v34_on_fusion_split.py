...
def ensure_dirs():
    ...
    os.makedirs(bertweet_v34_checkpoints_dir, exist_ok=True)
    os.makedirs(bertweet_v34_logs_dir, exist_ok=True)
...
def load_train_rows_for_split(target: str, train_ids: set, val_ids: set) -> List[dict]:
    ...
    for row in iter_ndjson(bertweet_v34_train_tokenized_path):
        cid = str(row["celebrity_id"])
...
def load_test_rows(target: str) -> List[dict]:
    ...
    for row in iter_ndjson(bertweet_v34_test_tokenized_path):
        if target == "occupation_3class" and row["occupation"] == "creator":
            continue
        rows.append(row)
...
def train_one_target(target: str):
    print(f"\n========== HybridV4 BERTweet V3.4 on fusion split: target={target} ==========")
    ...
    val_predictions, val_metrics = aggregate_celebrity_predictions(
        ...
        prob_key="bertweet_v34_probabilities",
        pred_key="bertweet_v34_pred_label",
        split="fusion_val",
    )
    ...
    val_pred_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_fusion_val_bertweet_v34_probs.json",
    )
    test_pred_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_test_bertweet_v34_probs.json",
    )
    val_metrics_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_fusion_val_bertweet_v34_metrics.json",
    )
    test_metrics_path = os.path.join(
        hybrid_v4_bertweet_probs_dir,
        f"{target}_test_bertweet_v34_metrics.json",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Train HybridV4 BERTweet V3.4 on fusion split"
    )
    parser.add_argument(
        "--target",
