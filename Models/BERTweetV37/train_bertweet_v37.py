...
from _constants import bertweet_train_tokenized_path
...
BERTWEET_V37_LOGS_DIR = os.path.join(BERTWEET_V37_OUTPUT_DIR, "logs")
...
# -------------------------------------------------------------------
def ensure_dirs():
    os.makedirs(BERTWEET_V37_OUTPUT_DIR, exist_ok=True)
    os.makedirs(BERTWEET_V37_CHECKPOINTS_DIR, exist_ok=True)
    os.makedirs(BERTWEET_V37_LOGS_DIR, exist_ok=True)
...
def get_label(row: dict, target: str) -> str:
    ...
    if target == "creator_performer":
        if occupation not in {"creator", "performer"}:
            raise ValueError(
                "creator_performer received a non creator/performer row. "
                "Filter rows before training."
            )
        return occupation
...
def filter_rows_for_target(rows: List[dict], target: str) -> List[dict]:
    if target == "creator_performer":
        before = len(rows)
        rows = [row for row in rows if str(row["occupation"]) in {"creator", "performer"}]
        after = len(rows)
        print(f"[INFO] creator_performer: filtered non-entertainment rows {before} -> {after}")
        return rows

    # occupation_group3 keeps all rows and maps creator/performer to entertainment.
    return rows
...
def evaluate_celebrity_level(
    ...
    metrics = {
        ...
        "model_name": MODEL_NAME,
        "voting_strategy": VOTING_STRATEGY,
        "num_val_celebrities": int(len(set(str(row["celebrity_id"]) for row in val_rows))),
...
def build_training_args(target: str):
    output_dir = os.path.join(BERTWEET_V37_CHECKPOINTS_DIR, target)
    logging_dir = os.path.join(BERTWEET_V37_LOGS_DIR, target)
...
def train_one_target(target: str):
    print(f"\n========== BERTweet V3.7 TARGET: {target} ==========")
    set_seed(RANDOM_SEED)

    rows = load_json_or_ndjson(bertweet_train_tokenized_path)
    rows = filter_rows_for_target(rows, target)
...
def main():
    parser = argparse.ArgumentParser(description="Train BERTweet V3.7 hierarchical occupation models")
    parser.add_argument(
        "--target",
        choices=[
            "occupation",
