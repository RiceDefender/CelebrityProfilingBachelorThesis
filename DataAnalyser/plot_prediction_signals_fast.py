...
Useful variants:
  ...
  # Hide @USER / HTTPURL from n-gram plots but keep them in style features:
  python plot_prediction_signals.py --target occupation --split test --ngrams 2 --drop-social-ngram-tokens
...
DEFAULT_SOCIAL_NGRAM_TOKENS = {"@user", "httpurl"}
DEFAULT_ARTIFACT_TOKENS = {"rt", "via", "amp", "gt", "lt"}


def get_excluded_ngram_tokens(args: argparse.Namespace) -> set[str]:
    """Tokens that make an n-gram unhelpful for plots.

    This filters whole n-grams, not the underlying tweet text.
    Use it to hide artifacts like @user / HTTPURL in n-gram charts while
    keeping them available for style features.
    """
    tokens = {str(t).lower() for t in (args.exclude_ngram_tokens or [])}
    if args.drop_social_ngram_tokens:
        tokens.update(DEFAULT_SOCIAL_NGRAM_TOKENS)
    return tokens


def get_drop_token_set(args: argparse.Namespace) -> set[str]:
    tokens = set()
    if args.drop_rt_artifacts:
        tokens.update(DEFAULT_ARTIFACT_TOKENS)
    return tokens
...
def tokenize_for_ngrams(
    ...
    out = []
    drop_tokens = drop_tokens or set()
    for tok in tokens:
        plain = tok[1:] if tok.startswith("#") else tok
        if tok in drop_tokens or plain in drop_tokens:
            continue
...
def get_ngrams_from_texts(
    ...
    exclude_ngram_tokens: Optional[set[str]] = None,
    drop_tokens: Optional[set[str]] = None,
) -> List[str]:
    phrases: List[str] = []
    exclude_ngram_tokens = exclude_ngram_tokens or set()
    drop_tokens = drop_tokens or set()
    iterable = texts if max_tweets is None or max_tweets <= 0 else texts[:max_tweets]
    for t in iterable:
        tokens = tokenize_for_ngrams(t, min_token_len, remove_stopwords, keep_hashtags, drop_tokens=drop_tokens)
        if len(tokens) >= n:
            for i in range(len(tokens) - n + 1):
                gram_tokens = tokens[i : i + n]
                if exclude_ngram_tokens and any(tok in exclude_ngram_tokens for tok in gram_tokens):
                    continue
                phrases.append(" ".join(gram_tokens))
    return phrases


def style_features(texts: Sequence[str], max_tweets: Optional[int] = None) -> Dict[str, float]:
...
def build_ngram_count_cache(
    preds: Sequence[dict],
    feeds: Dict[str, List[str]],
    n: int,
    args: argparse.Namespace,
) -> Dict[str, Counter]:
    """Precompute one Counter per celebrity.

    This is much faster for log-odds because each feed is tokenized once per n,
    instead of once per group and once per background group.
    """
    exclude_tokens = get_excluded_ngram_tokens(args)
    drop_tokens = get_drop_token_set(args)
    cache: Dict[str, Counter] = {}
    ids = [str(p.get("celebrity_id")) for p in preds]
    for cid in ids:
        texts = feeds.get(cid)
        if not texts or cid in cache:
            continue
        cache[cid] = Counter(
            get_ngrams_from_texts(
                ...
                exclude_ngram_tokens=exclude_tokens,
...
def sum_counters_for_ids(ids: Iterable[str], cache: Dict[str, Counter]) -> Counter:
    c: Counter = Counter()
    for cid in ids:
        cnt = cache.get(cid)
        if cnt:
...
def counter_difference(total: Counter, remove: Counter) -> Counter:
    rest = total.copy()
    rest.subtract(remove)
    # Remove zero/negative entries after subtracting.
    rest += Counter()
    return rest
...
def collect_counts_for_group(
    ...
    n: int,
    args: argparse.Namespace,
    cache: Optional[Dict[str, Counter]] = None,
) -> Counter:
    if cache is not None:
        return sum_counters_for_ids((str(p.get("celebrity_id")) for p in selected_preds), cache)
...
def run_frequency(
    ...
    for n in args.ngrams:
        cache = build_ngram_count_cache(preds, feeds, n, args)
        group_rows: Dict[str, List[Tuple[str, float]]] = {}
        for spec in specs:
            selected = select_top(preds, spec, args.top_docs)
            counts = collect_counts_for_group(selected, feeds, n, args, cache=cache)
...
def run_logodds(
    ...
    for n in args.ngrams:
        ...
        if args.max_rest_docs and args.max_rest_docs > 0:
            background_ids = list(all_ids)[: args.max_rest_docs]
        else:
            background_ids = list(all_ids)
        global_counts = sum_counters_for_ids(background_ids, cache)
        ...
        for spec in specs:
            ...
            group_counts = sum_counters_for_ids(group_ids, cache)
            rest_counts = counter_difference(global_counts, group_counts)
...
def run_style(
    ...
    for cid, p in pred_by_id.items():
        ...
        if not texts:
            continue
        feats = style_features(texts, max_tweets=args.max_tweets_per_celebrity)
        row = {
            "celebrity_id": cid,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--exclude-ngram-tokens",
        nargs="*",
        default=[],
        help="Drop any n-gram containing one of these tokens, e.g. --exclude-ngram-tokens @user httpurl",
    )
    parser.add_argument(
        "--drop-social-ngram-tokens",
        action="store_true",
        help="Shortcut for --exclude-ngram-tokens @user httpurl. Style features still keep them.",
    )
    parser.add_argument(
        "--drop-rt-artifacts",
        action="store_true",
        help="Remove RT/via/amp/gt/lt tokens before creating n-grams.",
    )
