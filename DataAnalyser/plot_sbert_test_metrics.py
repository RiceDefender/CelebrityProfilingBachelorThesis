...
from _constants import sbert_output_dir
...
def ensure_dirs():
    test_metrics_dir = Path(sbert_output_dir) / "test_metrics"
    plots_dir = test_metrics_dir / "plots"
    tables_dir = test_metrics_dir / "tables"

    plots_dir.mkdir(parents=True, exist_ok=True)
    tables_dir.mkdir(parents=True, exist_ok=True)

    return test_metrics_dir, plots_dir, tables_dir


def load_predictions(target: str) -> pd.DataFrame:
    path = Path(sbert_output_dir) / "predictions" / f"{target}_test_predictions.json"
...
def plot_confusion_matrix(y_true, y_pred, labels, title, output_path, normalize=False):
...
def evaluate_target(target: str, plots_dir: Path, tables_dir: Path):
    ...
    prefix = f"sbert_test_{target}"

    # Predictions CSV
    df.to_csv(tables_dir / f"{prefix}_predictions.csv", index=False)
    ...
    # Confusion Matrix CSV normalized
    ...
    # Confusion Matrix PNG absolute
    plot_confusion_matrix(
        y_true=y_true,
        y_pred=y_pred,
        labels=labels,
        title=f"SBERT Test {target.capitalize()} Confusion Matrix",
        output_path=plots_dir / f"{prefix}_confusion_matrix.png",
        normalize=False,
    )

    # Confusion Matrix PNG normalized
    plot_confusion_matrix(
        y_true=y_true,
        y_pred=y_pred,
        labels=labels,
        title=f"SBERT Test {target.capitalize()} Confusion Matrix (Normalized)",
    ...
    with open(tables_dir.parent / f"{target}_test_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2, ensure_ascii=False)

    print(f"[OK] Created SBERT test metrics and confusion matrices for {target}")

    return metrics


def main():
    test_metrics_dir, plots_dir, tables_dir = ensure_dirs()

    summary_rows = []

    for target in TARGETS:
        metrics = evaluate_target(target, plots_dir, tables_dir)
    ...
    summary_df.to_csv(test_metrics_dir / "sbert_test_summary_metrics.csv", index=False)

    print(f"[OK] Summary saved to {test_metrics_dir / 'sbert_test_summary_metrics.csv'}")
